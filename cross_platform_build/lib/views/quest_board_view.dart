...
class _QuestBoardViewState extends State<QuestBoardView> {
  ...
  Widget _buildCustomRoutinesSection(UserProfileManager profile) {
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        for (final e in routine.exercises) {
                          final workout = SuggestedWorkout.allWorkouts
                              .firstWhere(
                                (w) => w.id == e.workoutId,
                                orElse: () =>
                                    SuggestedWorkout.allWorkouts.first,
                              );
                        ...
                        _showOutcomeDialog(
                          ...
                          message:
                              "You completed '${routine.name}' and logged ${routine.exercises.length} exercises!",
  ...
  Widget _buildSuggestedWorkoutsSection(UserProfileManager profile) {
    ...
    if (profile.activeWorkoutGoal != AdvancedWorkoutGoal.none &&
        _workoutSearchQuery.isEmpty) {
      ...
      if (progression.goal == profile.activeWorkoutGoal) {
        ...
        final prerequisiteWorkouts = SuggestedWorkout.allWorkouts
            .where(
              (w) =>
                  progression.prerequisiteWorkoutIds.contains(w.id) &&
                  !isWorkoutCompletedToday(w),
            )
            .toList();

        // Remove them from filtered if they are already there so we don't duplicate
        filtered.removeWhere(
          (w) => progression.prerequisiteWorkoutIds.contains(w.id),
        );
...
class _SuggestedWorkoutLogDialogState
  ...
  void initState() {
    ...
    _repsController = TextEditingController(
      text: widget.workout.reps.replaceAll(RegExp(r'[^0-9]'), ''),
    );

    // Auto-populate weight with bodyweight if it's a bodyweight exercise, otherwise 0 or empty.
    final bool isBodyweight = widget.workout.equipment.toLowerCase().contains(
      "bodyweight",
    );
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildField("Sets", _setsController)),
                const SizedBox(width: 12),
                Expanded(child: _buildField("Reps/Time", _repsController)),
              ],
            ),
            const SizedBox(height: 16),
            _buildField(
      ...
      actions: [
        ...
        TextButton(
          onPressed: () {
            final sets =
                int.tryParse(_setsController.text) ?? widget.workout.sets;
            String reps = _repsController.text.isNotEmpty
